...
import { Upload, Input, Button, DatePicker, message } from "antd";
...
const Banners = () => {
	...
	const mutation = useMutation({
		mutationFn: async (data) => {
			...
			const imageFile = data?.image?.originalFileObj
			const body = {
				title: data.title,
				url: data.url,
			...
			return await customFetch.post("/admin/upload-banner",  {...body});
		},
		onSuccess: () => {
	...
	return (
		<div className="max-w-xl mx-auto p-6 bg-white rounded shadow">
			...
			<form onSubmit={handleSubmit(onSubmit, onInvalid)} className="space-y-6">
					...
					<Controller
						...
						render={({ field }) => (
							...
							>
								...
								<p className="ant-upload-hint">Drag & Drop or Choose file to upload</p>
							</Dragger>
						)}
					/>
				...
				{/* Valid Until */}
				<div>
					<label className="block font-medium mb-1">Valid Until<span className="text-red-500">*</span></label>
					<Controller
						name="valid_until"
						control={control}
						render={({ field }) => (
